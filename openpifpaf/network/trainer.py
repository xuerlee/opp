...
class Trainer():
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--train-batches', default=28298, type=int,  #  number of images / batch size, no more than len(train_loader)
        # group.add_argument('--train-batches', default=29568, type=int,
        # group.add_argument('--train-batches', default=1, type=int,
                           help='number of train batches')  # 每个epoch训练几个batch
        group.add_argument('--val-batches', default=1, type=int,
        ...
        group.add_argument('--joint-train', default=True,
                           help='joint train cocodet and cocokp or not')
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        cls.joint_train = args.joint_train
    ...
    def train_batch(self, data, targets, apply_gradients=True):  # 输出train中每个batch的loss
        # print(targets.size())
        if self.device.type != 'cpu':
            ...
            with torch.autograd.profiler.record_function('to-device'):
                ...
                targets = [head.to(self.device, non_blocking=True)
                           ...
                           for head in targets]  # divide targets by heads
        ...
        with torch.autograd.profiler.record_function('loss'):
            '''
            target size:
            targets[0].size(), targets[1].size(), targets[2].size()

            kp-det:
            torch.Size([4, 17, 5, 49, 49]) batch, 17 keypoints, (intensity, offsetx, offsety, b, scale), h, w
            torch.Size([4, 19, 9, 49, 49]) batch, 19 connections, (intensity, b1, b2, offsetx1, offsety1, offset2, offsety2, scale1, scale2), h, w
            None;
            None
            None
            torch.Size([4, 91, 7, 33, 33]) batch, 91 classes, (intensity, offsetx, offsety, bboxh, bboxw, b, scale), h, w

            joint: (cif, caf, cifdet)
            torch.Size([4, 17, 5, 49, 49])
            torch.Size([4, 19, 9, 49, 49])
            torch.Size([4, 91, 7, 33, 33])
            '''
            loss, head_losses = self.loss(outputs, targets)
            # self.loss is defined in loss/factory, return the defination: loss = MultiHeadLoss(losses, component_lambdas)
            # 调用network/losses/multi_head中的fowrard，在其中用的是composite进行计算的
            if self.train_profile and self.device.type != 'cpu':
                torch.cuda.synchronize()
        if loss is not None:

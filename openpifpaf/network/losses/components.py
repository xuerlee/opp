...
class Bce(torch.nn.Module):
    ...
    def forward(self, x, t):  # pylint: disable=arguments-differ
        ...
        bce = torch.nn.functional.binary_cross_entropy_with_logits(
            x, t_zeroone, reduction='none')  # BCE losd
...
class BceDistance(Bce):
    def forward(self, x, t):
        t_sign = t.clone()
        t_sign[t > 0.0] = 1.0
        t_sign[t <= 0.0] = -1.0  # t: intensity from encoder

        # construct target location relative to x but without backpropagating through x
        x_detached = x.detach()
        focal_loss_modification = 1.0  # w(focal loss modification) x BCE
        ...
        if self.focal_gamma == 1.0:
            ...
            focal_loss_modification = focal_loss_modification * (p_bar + p * neg_ln_p)  # 多分类BCE
        # focal_loss_modification = focal_loss_modification * (1/(1+exp(t*x)) + (1-(1/(1+exp(t*x)))) * ((1/t)*log(1+exp(-t*x))))
...
class Scale(torch.nn.Module):
    ...
    def forward(self, x, t):  # pylint: disable=arguments-differ
        ...
        denominator = self.b  # 分母
        if self.relative:
            denominator = self.b * (self.relative_eps + t)
        d = d / denominator  # 1/b * distance
...
class RegressionDistance:
    ...
    def __call__(x_regs, t_regs, t_sigma_min, t_scales):
        ...
        d = torch.linalg.norm(d, ord=2, dim=3)  # L2 distance
...
class RegressionLoss:
    ...
    def __call__(x_regs, t_regs, t_sigma_min, t_scales):
        ...
        d = torch.cat([d, t_sigma_min_imputed], dim=2)  # + b

        # L2 distance for coordinate pair
        d = torch.linalg.norm(d, ord=2, dim=2, keepdim=True)  # L2 distance
        ...
        if t_scales.shape[1]:
            ...
            t_sigma = 0.5
        d = 1.0 / t_sigma * d  # t_sigma: b  # logb 加在composite处了

...
class MultiHeadLoss(torch.nn.Module):
    ...
    def __init__(self, losses, lambdas):
        ...
        if not lambdas:
            lambdas = [1.0 for l in losses for _ in l.field_names]  # l: single loss
        assert all(lam >= 0.0 for lam in lambdas)

        self.losses = torch.nn.ModuleList(losses)  # losses: calculation defination
        # losses: compositeloss, self.losses: modulelist
    ...
    def forward(self, head_fields, head_targets):  # pylint: disable=arguments-differ
        ...
        '''
        self.field_names:
        ['cocokp.cif.c', 'cocokp.cif.vec', 'cocokp.cif.scales', 'cocokp.caf.c', 'cocokp.caf.vec', 'cocokp.caf.scales', 'cocodet.cifdet.c', 'cocodet.cifdet.vec', 'cocodet.cifdet.scales']
        '''
        flat_head_losses = [ll
                            for l, f, t in zip(self.losses, head_fields, head_targets)
                            for ll in l(f, t)]  # self.losses就是composite函数  这里代表每个head设置的loss所得值集合, 所以应该是cif, caf, cifdet
        # call composite.py CompositeLoss forward

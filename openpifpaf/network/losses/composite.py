...
class CompositeLossByComponent(torch.nn.Module):
    ...
    def __init__(self, head_meta):
        ...
        self.field_names = (  # cocokp.cif, cocokp.caf, cocokp.dcaf, cocodet.cifdet  3x3=9?
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument('--regression-loss', default='laplace',
                           choices=['smoothl1', 'l1', 'laplace'],  # 用的是laplace loss
...
class CompositeLoss(torch.nn.Module):  # default loss
    ...
    def __init__(self, head_meta):
        ...
        self.n_vectors = head_meta.n_vectors  # cif 和 caf不同
        self.n_scales = head_meta.n_scales
    ...
    def forward(self, x, t):
        ...
        '''
        self.field_names:
        ('cocokp.cif.c', 'cocokp.cif.vec', 'cocokp.cif.scales')
        ('cocokp.caf.c', 'cocokp.caf.vec', 'cocokp.caf.scales')
        ('cocodet.cifdet.c', 'cocodet.cifdet.vec', 'cocodet.cifdet.scales')
        '''
        if t is None:
        ...
        scale_mask = torch.all(finite[:, :, :, :, self.n_confidences + self.n_vectors * 3:], dim=4)  # 把几个预测信息提取出来
        '''
        bgmask torch.Size([4, 17, 49, 49])
        bgmask torch.Size([4, 19, 49, 49])
        bgmask torch.Size([4, 91, 33, 33])
        '''
        # extract masked ground truth
        t_confidence_bg = t[bg_mask][:, 0:self.n_confidences]
        t_confidence = t[c_mask][:, 0:self.n_confidences]  # 前景和背景的conf
        t_regs = t[reg_mask][:, self.n_confidences:1 + self.n_vectors * 2]  # offset
        ...
        '''
        x torch.Size([4, 17, 49, 49, 5])
        x torch.Size([4, 19, 49, 49, 8])
        x torch.Size([4, 91, 33, 33, 6])
        '''
        x_confidence_bg = x[bg_mask][:, 1:1 + self.n_confidences]
        x_logs2_c = x[c_mask][:, 0:1]
        x_confidence = x[c_mask][:, 1:1 + self.n_confidences]  # conf
        x_logs2_reg = x[reg_mask][:, 0:1]  # 用于logb b在encoder中没有标注，是网络自己学到的
        x_regs = x[reg_mask][:, 1 + self.n_confidences:1 + self.n_confidences + self.n_vectors * 2]  # offset
        # x_logs2_scale = x[scale_mask][:, 0:1]
        x_scales_reg = x[reg_mask][:, 1 + self.n_confidences + self.n_vectors * 2:]
        x_scales = x[scale_mask][:, 1 + self.n_confidences + self.n_vectors * 2:]  # scale
        ...
        l_confidence = self.bce_loss(x_confidence, t_confidence)  # conf loss
        l_reg = self.reg_loss(x_regs, t_regs, t_sigma_min, t_scales_reg)  # offset loss
        l_scale = self.scale_loss(x_scales, t_scales)  # scale loss
        ...
        l_confidence = 0.5 * l_confidence * torch.exp(-x_logs2_c) + 0.5 * x_logs2_c  # conf loss: Bce distance + ?
        # reg
        ...
        l_reg = l_reg * reg_factor + x_logb  # offset loss： L2 distance + logb
        # scale

...
#: headmeta class to head class   通过headmeta关键字来定义头网络？？？
HEADS = {
...
class Factory(Configurable):
    ...
    checkpoint = 'output_test/shufflenetv2k16-241005-014657-cocojoint.pkl.epoch272'  # 从预训练模型中获取网络架构
    ...
    head_consolidation = 'filter_and_extend'  # following the head in datasets meta
    cross_talk = None
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        for bn in BASE_TYPES:
            bn.cli(parser)  # 添加关于骨干网络的参数组  network/basenetworks.py
        for hn in set(HEADS.values()):
            hn.cli(parser)  # 添加关于检测头的参数组  network/heads.py

        group = parser.add_argument_group('network configuration')  # 添加关于network的参数组
    ...
    def configure(cls, args: argparse.Namespace):
        for bn in BASE_TYPES:
            bn.configure(args)  # 每一个基础网络都定义一遍
        for hn in set(HEADS.values()):
            hn.configure(args)
    ...
    def factory(self, *, head_metas=None) -> Tuple[nets.Shell, int]:
        # define in train.py, datamodules are defined in datasets/factory.py -- multimodule.py
        ...
        net_cpu, epoch = self.from_checkpoint()  # 根据base_name或checkpoint定义骨干网络或从预训练的权重中提取骨干网络
    ...
    def consolidate_heads(self, net_cpu, head_metas):
        if self.head_consolidation == 'keep':
            ...
            existing_headnets = {(hn.meta.dataset, hn.meta.name): hn
                                 for hn in net_cpu.head_nets}
            headnets = []  # following metas in datasets settings
    ...
    def from_checkpoint(self) -> Tuple[nets.Shell, int]:
        ...
        net_cpu: nets.Shell = checkpoint['model']  # 提取骨干网络
        epoch = checkpoint['epoch']  # 提取已训练批次

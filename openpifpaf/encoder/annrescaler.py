...
'''
label处理，dataloader中的一部分
'''
'''
1 首先对ann的值进行scale操作. 因为网络的输出和输入大小不一致, 而传入的ann是按照输入的大小计算得到的值, 
因此需要根据网络的缩放倍数对ann值进行更新, 符合网络的输出尺度范围, 就是对象rescalar所执行的作用. 所以输入时的预定义pose是因为这个？
2 rescalar. 除了对ann里面的keypoints和valid_area进行缩放外, 还会额外生成一个bg_mask. 
...
class AnnRescaler():
    ...
    def __init__(self, stride, pose=None):
        self.stride = stride   # none
        self.pose = pose  # cocokp中有定义,一些预定义的关键点位置
        ...
        if pose is not None:
            ...
            self.pose_45[:, :2] = np.einsum('ij,kj->ki', rotate, self.pose_45[:, :2])  # 爱因斯坦求和相乘再相加
    ...
    def valid_area(self, meta):
        # 变换img和ann之后，其中得到的meta['valid_area']值对image再做一次更新, 就是让image处于valid_area之外的区域值为0,
        # 意思就是让网络专注于去学只有人的区域, 非人的区域不要去学, 接着根据得到的ann值去生成pif label 和 paf label.
        if 'valid_area' not in meta:  # 好像都没有
            return None
    ...
    def suppress_collision_(keypoint_sets_bbox):
        for p_i, (kps_p, bbox_p) in enumerate(keypoint_sets_bbox[:-1]):  # 提取keypoints和bbox
            for kps_s, bbox_s in keypoint_sets_bbox[p_i + 1:]:  # 逐一过滤附近target
                d_th = 0.2 * max(bbox_p[2], bbox_p[3], bbox_s[2], bbox_s[3])  # 选择bbox的边界？
                d_th = max(16.0, d_th)
                diff = np.abs(kps_p[:, :2] - kps_s[:, :2])  # 两个关键点的距离？
                collision = (
                    (kps_p[:, 2] > 0.0)
                    & (kps_s[:, 2] > 0.0)
                    & (diff[:, 0] < d_th)
                ...
                if np.any(collision):
                    ...
                    kps_s[collision, 2] = 0.0  # 如果有重合就复赋值为0？（暂时理解）
    ...
    def bg_mask(self, anns, width_height, *, crowd_margin):
        ...
        ), dtype=bool)  # mask的最大的大小
        for ann in anns:
            if not ann['iscrowd']:
                valid_keypoints = 'keypoints' in ann and np.any(ann['keypoints'][:, 2] > 0)  # 将所有keypoint都考虑
                if valid_keypoints:
                    continue
            ...
            if 'mask' not in ann:
                bb = ann['bbox'].copy()
                bb /= self.stride
                bb[2:] += bb[:2]  # convert width and height to x2 and y2  # xywh2xxyy
                ...
                mask[top:bottom, left:right] = 0  # 为什么赋值为0？
                continue
    ...
    def scale(self, keypoints):
        ...
        scale = np.sqrt(area) * factor_clipped  # 根据可不可见放缩后的pose area
        if scale < 0.1:
            scale = np.nan

        LOG.debug('instance scale = %.3f (factor = %.2f, clipped factor = %.2f)',
                  scale, factor, factor_clipped)
        return scale  # 这里返回的是关键点的大小


class AnnRescalerDet():  # 用于检测

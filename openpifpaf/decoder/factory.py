...
from openpifpaf.decoder.cifcaf import CifCaf, CifCafDense
...
from openpifpaf.decoder.track_base import TrackBase
from openpifpaf.decoder.tracking_pose import TrackingPose
from openpifpaf.decoder import utils
from openpifpaf.profiler import Profiler  # , TorchProfiler
...
def cli(parser, *, workers=None):
    ...
    group.add_argument('--decoder', default=['cifcaf:0', 'cifdet:0'], nargs='+',
...
class Factory:
        ...
        def per_class(request, dec_class):
            ...
            if request is not None \
               and class_name not in request:
                return []
            decoders = sorted(dec_class.factory(head_metas), key=lambda d: d.priority, reverse=True)  # 根据DECODERS建立decoder
            for dec_i, dec in enumerate(decoders):
                dec.request_index = dec_i
            if request is not None:
                indices = set(request[class_name])
                decoders = (d for i, d in enumerate(decoders) if i in indices)  # 返回request的decoder函数
            return decoders  # 返回一些列decoder函数
    ...
    def __call__(cls, head_metas):
        ...
        decoders = cls.decoders(head_metas)  # decoder函数(headmeta)  # predictor调用factory时自动返回所需的decoder函数

        if cls.profile:
            decode = decoders[0]
            decode.__class__.__call__ = Profiler(
                decode.__call__, out_name=cls.profile)  # 调用所定义的decoder函数

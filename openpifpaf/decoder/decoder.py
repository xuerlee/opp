...
from openpifpaf import annotation, visualizer
...
# 被predictor调用（事先也在predictor中用factory函数定义了）
class Decoder:
    ...
    def fields_batch(cls, model, image_batch, *, device=None):
        ...
        with torch.no_grad():
            ...
            with torch.autograd.profiler.record_function('model'):
                heads = model(image_batch)  # model的推理结果
    ...
    def batch(self, model, image_batch, *, device=None, gt_anns_batch=None):  # 被predictor调用，dataloader  # 怎么调用的decoder?
        """From image batch straight to annotations batch."""
        start_nn = time.perf_counter()
        fields_batch = self.fields_batch(model, image_batch, device=device)  # 生成model的推理结果

...
class RmKp(DataModule):  # 可以参考这个建立新的datamoudule
    """
    DataModule for the Apollocar3d Dataset.
    """

    train_annotations = 'robo-kp/annotations/apollo_keypoints_66_train.json'
    val_annotations = 'robo-kp/annotations/apollo_keypoints_66_val.json'
    eval_annotations = val_annotations
    train_image_dir = 'robo-kp/images/train/'
    val_image_dir = 'robo-kp/images/val/'
    ...
    def __init__(self):
        ...
        if self.weights is not None:
            ...
            caf_weights = None
        cif = headmeta.Cif('cif', 'robomaster',
        ...
        caf = headmeta.Caf('caf', 'robomaster',
                           keypoints=self.CAR_KEYPOINTS,
                           sigmas=self.CAR_SIGMAS,
                           pose=self.CAR_POSE,
        ...
        self.head_metas = [cif, caf]  # head network

    @classmethod
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module Apollo')

        group.add_argument('--rm-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--rm-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--rm-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--rm-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--rm-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--rm-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--rm-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--rm-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--rm-no-augmentation',
                           dest='rm_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--rm-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--rm-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--rm-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--rm-bmin',
                           default=cls.b_min, type=int,
                           help='b minimum in pixels')
        group.add_argument('--rm-apply-local-centrality-weights',
                           dest='rm_apply_local_centrality',
                           default=False, action='store_true',
                           help='Weigh the CIF and CAF head during training.')

        # evaluation
        assert cls.eval_annotation_filter
        group.add_argument('--rm-no-eval-annotation-filter',
                           dest='rm_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--rm-eval-long-edge', default=cls.eval_long_edge, type=int,
                           help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--rm-eval-extended-scale', default=False, action='store_true')
        group.add_argument('--rm-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float)
        group.add_argument('--rm-use-24-kps', default=False, action='store_true',
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # Apollo specific
        cls.train_annotations = args.rm_train_annotations
        cls.val_annotations = args.rm_val_annotations
        cls.eval_annotations = cls.val_annotations
        cls.train_image_dir = args.rm_train_image_dir
        cls.val_image_dir = args.rm_val_image_dir
        cls.eval_image_dir = cls.val_image_dir

        cls.square_edge = args.rm_square_edge
        cls.extended_scale = args.rm_extended_scale
        cls.orientation_invariant = args.rm_orientation_invariant
        cls.blur = args.rm_blur
        cls.augmentation = args.rm_augmentation  # loaded by the dest name
        cls.rescale_images = args.rm_rescale_images
        cls.upsample_stride = args.rm_upsample
        cls.min_kp_anns = args.rm_min_kp_anns
        cls.b_min = args.rm_bmin
        if args.rm_use_24_kps:
        ...
        # evaluation
        cls.eval_annotation_filter = args.rm_eval_annotation_filter
        cls.eval_long_edge = args.rm_eval_long_edge
        cls.eval_orientation_invariant = args.rm_eval_orientation_invariant
        cls.eval_extended_scale = args.rm_eval_extended_scale
        if args.rm_apply_local_centrality:
            if args.rm_use_24_kps:

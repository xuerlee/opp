...
class CocoDataset_multiann(torch.utils.data.Dataset):
    """`MS Coco Detection <http://mscoco.org/dataset/#detections-challenge2016>`_ Dataset.
    ...
    def __init__(self, image_dir, ann_file_kp, ann_file_det, *,
                 preprocess_kp=None, preprocess_det=None, min_kp_anns=0,
                 category_ids=None,
                 annotation_filter=False):
        super().__init__()
        ...
        from pycocotools.coco import COCO  # pylint: disable=import-outside-toplevel
        self.image_dir = image_dir
        self.coco_kp = COCO(ann_file_kp)
        self.coco_det = COCO(ann_file_det)

        self.category_ids = category_ids

        self.ids = self.coco_kp.getImgIds(catIds=self.category_ids)
        if annotation_filter:
            self.filter_for_annotations(min_kp_anns=min_kp_anns)
        elif min_kp_anns:
            raise Exception('only set min_kp_anns with annotation_filter')
        ...
        self.preprocess_kp = preprocess_kp or openpifpaf.transforms.EVAL_TRANSFORM
        self.preprocess_det = preprocess_det or openpifpaf.transforms.EVAL_TRANSFORM

    def filter_for_annotations(self, *, min_kp_anns=0):
        LOG.info('filter for annotations (min kp=%d) ...', min_kp_anns)

        def filter_image(image_id):
            ann_ids_kp = self.coco_kp.getAnnIds(imgIds=image_id, catIds=self.category_ids)
            # ann_ids_det = self.coco_det.getAnnIds(imgIds=image_id, catIds=self.category_ids)
            anns_kp = self.coco_kp.loadAnns(ann_ids_kp)
            # anns_det = self.coco_det.loadAnns(ann_ids_det)
            anns_kp = [ann_kp for ann_kp in anns_kp if not ann_kp.get('iscrowd')]
            # anns_det = [ann_det for ann_det in anns_det if not ann_det.get('iscrowd')]
            if not anns_kp:
                return False
            kp_anns = [ann_kp for ann_kp in anns_kp
                       if 'keypoints' in ann_kp and any(v > 0.0 for v in ann_kp['keypoints'][2::3])]
            return len(kp_anns) >= min_kp_anns
    ...
    def __getitem__(self, index):
        ...
        anns_kp = self.coco_kp.loadAnns(ann_ids_kp)
        anns_det = self.coco_det.loadAnns(ann_ids_det)
        anns_kp = copy.deepcopy(anns_kp)
        anns_det = copy.deepcopy(anns_det)

        image_info = self.coco_det.loadImgs(image_id)[0]
        LOG.debug(image_info)
        local_file_path = os.path.join(self.image_dir, image_info['file_name'])
        with open(local_file_path, 'rb') as f:
            image = Image.open(f).convert('RGB')
        ...
        if 'flickr_url' in image_info:
            ...
            flickr_id, _ = flickr_file_name.split('_', maxsplit=1)
            meta['flickr_full_page'] = 'http://flickr.com/photo.gne?id={}'.format(flickr_id)
        # preprocess image and annotations to get encoded data
        image_aug1, anns_kp, meta1 = self.preprocess_kp(image, anns_kp, meta)
        image_aug2, anns_det, meta2 = self.preprocess_det(image, anns_det, meta)
        ...
        anns = anns_kp + anns_det
        # anns = anns_kp

        # print("anns_det", len(anns))

        return image_aug1, anns, meta

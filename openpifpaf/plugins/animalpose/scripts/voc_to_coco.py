...
# 将混合来源的数据集转换为json(COCO)的数据格式 应该是针对之前的数据集分布的
...
def cli():
    ...
    parser.add_argument('--dir_data', default='/home/xuer/shuju/data-animalpose',
                        help='dataset directory')
    parser.add_argument('--dir_out', default='/home/xuer/shuju/data-animalpose',
...
class VocToCoco:
    ...
    def __init__(self, dir_dataset, dir_out, args):
        ...
        self.dir_out_im = os.path.join(dir_out, 'images')  # 将两个来源的数据集转换成一致
        ...
        os.makedirs(os.path.join(self.dir_out_im, 'train'))  # 分成train和val
        os.makedirs(os.path.join(self.dir_out_im, 'val'))
        self.sample = args.sample

    def process(self):
        splits = self._split_train_val()  # 图片和标注的路径
        all_xml_paths = []
        for phase in ('train', 'val'):
            ...
            for (im_path, im_id, xml_paths) in metadata:
                self._process_image(im_path, im_id)  # 图片信息写入json
                cnt_images += 1
    ...
    def _process_annotation(self, xml_path, im_id):
        ...
        root = tree.getroot()
        box_obj = root.findall('visible_bounds')  # bounding box

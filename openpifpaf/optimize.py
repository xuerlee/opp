...
def cli(parser):
    ...
    group.add_argument('--no-nesterov', dest='nesterov', default=False, action='store_false',
    ...
    group_s.add_argument('--lr', type=float, default=1e-4,
                         help='learning rate')
    group_s.add_argument('--lr-decay', default=[380, 390], nargs='+', type=float,
    ...
    group_s.add_argument('--lr-warm-up-epochs', default=0, type=float,
...
def factory_lrscheduler(args, optimizer, training_batches_per_epoch, last_epoch=0):
    LOG.info('training batches per epoch = %d', training_batches_per_epoch)  # len(train_loader)
    if last_epoch > 0:
